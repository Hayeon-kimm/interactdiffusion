...
def run_batch(meta, config, starting_noise=None):
    ...
    for i in tqdm(range(len(meta))):
        ...
        for image_id, sample in zip(image_ids, samples_fake):
            ...
            # Draw subject boxes on the image
            for box in grounding_input['subject_boxes'][0]:
                if not torch.all(box == 0):  # Check if the box is not empty
            ...
            for box in grounding_input['object_boxes'][0]:
                if not torch.all(box == 0):  # Check if the box is not empty
                    # Scale box coordinates to image dimensions
                    scaled_box = [
                        box[0] * img_width,
...
if __name__ == "__main__":
    ...
    if args.res == "hico":
        res_path = "DATA/hico_det_test_sg3.pkl" # 여기서 res가져옴
    else:
