...
def run(meta, config, starting_noise=None):
    ...
    print(image_ids)
    subject_boxes = meta["subject_boxes"]
    object_boxes = meta["object_boxes"]
    for image_id, sample in zip(image_ids, samples_fake):
        ...
        img_width, img_height = sample.size

        for bbox in subject_boxes:
            draw.rectangle(
        ...
        sample.save(os.path.join(output_folder, meta['prompt']+img_name))


if __name__ == "__main__":

    parser = argparse.ArgumentParser()
    parser.add_argument("--folder", type=str, default="exp3_2", help="root folder for output")
    ...
    parser.add_argument("--negative_prompt", type=str,
                        ...
                        help="")
    parser.add_argument("--ckpt", type=str, default="/data2/rlagkdus705/interactdiffusion/0704_seg3/test/tag01/checkpoint_latest.pth") # seg3
    ...
    meta_list = [
        dict(
            ckpt=args.ckpt,
            prompt="a person is carrying a dining table",
            subject_phrases=['person'],
            object_phrases=['dining table'],
            action_phrases=['carrying'],
            subject_boxes=[[0.38093333333333335, 0.11625, 0.8426, 0.7113690476190476]],
            object_boxes=[[0.09183333333333334, 0.011369047619047619, 0.5179666666666667, 0.7325]],
            alpha_type=[0.9, 0.0, 0.1],
            save_folder_name="hico"
        ),
        dict(
            ckpt=args.ckpt,
            prompt="a person is hugging a laptop",
            subject_phrases=['person'],
            object_phrases=['laptop'],
            action_phrases=['hugging'],
            subject_boxes=[[0.2682843137254902, 0.16909313725490194, 0.6894117647058824, 0.9991666666666668]],
            object_boxes=[ [0.37722222222222224, 0.37884803921568627, 0.6010620915032681, 0.7942892156862745]],
            alpha_type=[0.9, 0.0, 0.1],
            save_folder_name="hico"
        ),
        dict(
            ckpt=args.ckpt,
            prompt="a person is kissing a bear",
            subject_phrases=['person'],
            object_phrases=['bear'],
            action_phrases=['kissing'],
            subject_boxes=[[0.0, 0.19588832487309646, 0.39928813559322035, 1.0]],
            object_boxes=[[0.24889830508474575, 0.06581218274111675, 0.9639322033898305, 1.0]],
            alpha_type=[0.9, 0.0, 0.1],
            save_folder_name="hico"
        ),
        dict(
            ckpt=args.ckpt,
            prompt="a person is repairing a refrigerator",
            subject_phrases=['person'],
            object_phrases=['refrigerator'],
            action_phrases=['repairing'],
            subject_boxes=[[0.38093333333333335, 0.11625, 0.8426, 0.7113690476190476]],
            object_boxes=[[0.09183333333333334, 0.011369047619047619, 0.5179666666666667, 0.7325]],
            alpha_type=[0.9, 0.0, 0.1],
            save_folder_name="hico"
        ...
        dict(
            ckpt=args.ckpt,
            prompt="a person is hugging a handbag",
            subject_phrases=['person'],
            object_phrases=['handbag'],
            action_phrases=['hugging'],
            subject_boxes=[[0.27355999999999997, 0.11138964577656676, 0.7885533333333333, 0.9992007266121709]],
            object_boxes=[[0.34934666666666664, 0.3455404178019982, 0.6357866666666667, 0.7731789282470481]],
            alpha_type=[0.9, 0.0, 0.1],
            save_folder_name="hico"
        ),
    ]
